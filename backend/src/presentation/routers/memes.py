...
def get_similar(meme_id: uuid.UUID, db: Session = Depends(get_db)) -> SimilarResponse:
    ...
    items = [MemeSummary.model_validate(m) for m in memes]
...
def create_download(
    ...
    download_cnt = EngageService(db).download(meme_id, ip_hash=get_ip_hash(request))
    return EngageResponse(meme_id=meme_id, download_cnt=download_cnt)
